...
_Fix._ Trailing whitespace is invisible but shows up as diff noise.
...
_Fix._ Use spaces, not hard tabs.
...
_Fix._ Files should end with exactly one newline.
...
_Flag._ Heading levels should increase one at a time.
...
_Fix._ Headings use one space after the marker and no closing #s.
...
_Fix._ Use `-` for unordered list markers.
...
_Fix._ Use one space after a list marker.
...
_Fix._ Indent nested list items to their parent's content.
...
_Fix._ Blank lines around blocks and headings are kept consistent.
...
_Flag._ Use a single top-level heading per document.
