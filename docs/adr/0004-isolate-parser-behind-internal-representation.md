...
This is the practical follow-through on [ADR 0002](./0002-comrak-parser.md), which noted that swapping parsers would otherwise touch every structural rule.
With this boundary, replacing comrak (or adding a second parser) becomes another implementation of the interface rather than a change rippling through every file.
